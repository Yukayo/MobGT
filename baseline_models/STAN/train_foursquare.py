...
def calculate_ndcg(prob, label):
    ...
    for i, k in enumerate([1, 5, 10, 20]):
        ...
        for j, topk_predict in enumerate(to_npy(topk_predict_batch)):
            # topk_predict (k)
            if to_npy(label)[j] in topk_predict:
                rank_index = topk_predict.tolist().index(to_npy(label)[j])
                ndcg_train[i] += 1.0 / np.log2(rank_index + 2)
    return np.array(ndcg_train)

def calculate_mrr(prob, label):
    # log_prob (N, L), label (N), batch_size [*M]
    mrr_train = [0]
    rec_list = prob.to("cpu").detach().numpy().argsort()[-len(prob):][0][::-1]
    r_idx = np.where(rec_list == label.to("cpu").detach().numpy())[0][0]
    mrr_train[0]+=1 / (r_idx + 1)
    return np.array(mrr_train)
...
class Trainer:
    ...
    def train(self):
        ...
        for t in range(self.num_epoch):
            ...
            for step, item in enumerate(self.data_loader):
                ...
                for mask_len in range(1, person_traj_len[0]+1):  # from 1 -> len
                    ...
                    if mask_len <= person_traj_len[0] - 2:  # only training
                        ...
                        ndcg_valid += calculate_ndcg(prob, train_label)
                        ...
                        ndcg_test += calculate_ndcg(prob, train_label)
            ...
            print('epoch:{}, time:{}, valid_mrr:{}'.format(self.start_epoch + t, time.time() - start, mrr_valid))
            ...
            print('epoch:{}, time:{}, test_mrr:{}'.format(self.start_epoch + t, time.time() - start, mrr_test))
            ...
            self.records['mrr_valid'].append(mrr_valid)
            self.records['mrr_test'].append(mrr_test)
            ...
            if self.threshold < np.mean(acc_valid):
                ...
                torch.save({'state_dict': self.model.state_dict(),
                           ...
                           './best_results/' + dname + '/best_stan_win_1000_' + str(end_part) + '_' + dname + '.pth')
            if t==self.num_epoch-1 or np.mean(acc_test)>best_acc:
                best_acc=np.mean(acc_test)
                if t!=self.num_epoch-1:
                    best_epoch=t
                if t==self.num_epoch-1:
                    if np.mean(acc_test)>best_acc:
                        best_epoch=t
                    print('best epoch:{}'.format(self.start_epoch + best_epoch))
...
if __name__ == '__main__':
    # load data
    dname = 'foursquare'
    file = open('../../dataset/baseline_models_dataset/STAN/' + dname + '_data.pkl', 'rb')
    ...
    print("All data length:{length} ".format(length=len(trajs)))
    start_part= 200
    end_part = 300
    part = end_part-start_part
    trajs, mat1, mat2t, labels, lens = trajs[start_part:end_part], mat1[start_part:end_part], mat2t[start_part:end_part], labels[start_part:end_part], lens[start_part:end_part]
    ...
    if load:
        checkpoint = torch.load('./best_results/{}/best_stan_win_{}.pth'.format(dname))
        stan.load_state_dict(checkpoint['state_dict'])
        start = time.time() - checkpoint['time']
        records = checkpoint['records']
    else:
        records = {'epoch': [], 'acc_valid': [], 'acc_test': [], 'ndcg_valid': [], 'ndcg_test': [], 'mrr_valid': [], 'mrr_test': []}
        start = time.time()
