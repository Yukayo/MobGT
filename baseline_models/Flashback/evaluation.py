...
class Evaluation:
    ...
    def evaluate(self):
        ...
        with torch.no_grad():        
            ...
            for i, (x, t, s, y, y_t, y_s, reset_h, active_users) in enumerate(self.dataloader):
                ...
                for j in range(self.setting.batch_size):  
                    ...
                    for k in range(len(y_j)):                    
                        ...
                        if t in r[:1]:
                            u_ndcg1[active_users[j]]+=1
                        
                        if t in r[:5]:
                            rank_index = r.tolist().index(t)
                        ...
                        rec_list = o_n[k].argsort()[-len(o_n[k]):][::-1]
                        r_idx = np.where(rec_list == t.to("cpu").detach().numpy())[0][0]
                        u_mrr[active_users[j]]+=1 / (r_idx + 1)
            ...
            print('Mrr:', formatter.format(mrr/iter_cnt))
