...
class Setting:
    ...
    def parse(self):
        ...
        self.guess_toyota = any(['toyota' in argv for argv in sys.argv])
                
        parser = argparse.ArgumentParser()        
        if self.guess_foursquare:
            self.parse_foursquare(parser)
        elif self.guess_toyota:
            self.parse_toyota(parser)
        else:
        ...
        # data management
        self.dataset_file = '../../dataset/baseline_models_dataset/Flashback/{}.txt'.format(args.dataset)
        self.max_users = 0 # 0 = use all available users
        
        self.batch_size = args.batch_size
        self.min_checkins = 10
    ...
    def parse_arguments(self, parser):        
        ...
        # data management
        parser.add_argument('--dataset', default='gowalla.txt', type=str, help='the dataset under ./data/<dataset.txt> to load')
    ...
    # TEST - the arguments are just a copy from parse_gowalla
    def parse_toyota(self, parser):
        # defaults for toyota dataset
        parser.add_argument('--batch-size', default=200, type=int, help='amount of users to process in one pass (batching)')
        parser.add_argument('--lambda_t', default=0.1, type=float, help='decay factor for temporal data')
        parser.add_argument('--lambda_s', default=1000, type=float, help='decay factor for spatial data')
    ...
    def __str__(self):        
        if self.guess_foursquare:
            str = 'parse with foursquare default settings'
        elif self.guess_toyota:
            str = 'parse with toyota default settings'
        else:
            str = 'parse with gowalla default settings'
