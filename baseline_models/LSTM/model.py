...
class TrajPreLocalAttnLong(nn.Module):
    ...
    def __init__(self, parameters):
        ...
        input_size = self.loc_emb_size + self.tim_emb_size
        self.attn = Attn(self.attn_type, self.hidden_size) #attn_type:dot 这个相当于调用init,下面的是调用forward
        self.fc_attn = nn.Linear(input_size, self.hidden_size)
        ...
        self.fc_final = nn.Linear(2 * self.hidden_size, self.loc_size) #输入1000，输出9296
        self.fc_final_RNN = nn.Linear(self.hidden_size, self.loc_size)
        self.dropout = nn.Dropout(p=parameters.dropout_p)
        self.init_weights()
    ...
    def init_weights(self):
        ...
        Here we reproduce Keras default initialization weights for consistency with Keras version
        """
        #看起来就是记录模型weight_ih/hh/bias的初始值/权重
        ih = (param.data for name, param in self.named_parameters() if 'weight_ih' in name) #weight_ih,hh,bias似乎都是LSTM拥有的权重/层/参数？
        hh = (param.data for name, param in self.named_parameters() if 'weight_hh' in name)
        b = (param.data for name, param in self.named_parameters() if 'bias' in name)
        ...
        #torch.nn.init.xavier_uniform_(tensor, gain=1)均匀分布 用一个均匀分布生成值，填充输入的张量或变量。结果张量中的值采样自U(-a, a)
        #torch.nn.init.xavier_normal_(tensor, gain=1)正态分布
        for t in hh:
            nn.init.orthogonal(t) #用（半）正交矩阵填充输入的张量或变量。输入张量必须至少是2维的，对于更高维度的张量，超出的维度会被展平，视作行等于第一个维度，列等于稀疏矩阵乘积的2维表示
        for t in b:
            nn.init.constant(t, 0) #nn.init.constant(tensor, val) 用val的值填充输入的张量或变量
    ...
    def forward(self, loc, tim, target_len):
        ...
        if self.rnn_type == 'GRU' or self.rnn_type == 'RNN':
            hidden_history, h1 = self.rnn_encoder(x[:-target_len], h1) #-target_len表示列表从后往前数第target_len个元素, 就是取前len-target_len个元素
