...
def caculate_poi_distance(poi_coors):
    ...
    pickle.dump(sim_matrix, open('../../dataset/baseline_models_dataset/LSTPM/tky_distance', 'wb'))
    return sim_matrix
...
def train_network(network, num_epoch=40 ,batch_size = 32,criterion = None):
    ...
    # output_df.to_csv("/fast/xuxh/xuxh/LSTPM-master/LSTPM-master/output/tky_output_last.txt")
    output_df.to_csv("./output/foursquare_output_prednew.txt")
...
def get_acc1(target, scores):
    ...
    val, idxx = scores[-1].unsqueeze(0).data.topk(10, 1)
    predx = idxx[0].cpu().numpy()
    acc = np.zeros((3, 1))
    ndcg = np.zeros((3, 1))
    t = target
...
def MRR_metric(target, scores):
    """Mean Reciprocal Rank: Reciprocal of the rank of the first relevant item """
    y_true = target[-1].data.cpu().numpy()
    y_pred = scores[-1].data.cpu().numpy()
    mrr=0
    ...
    with torch.no_grad():
        ...
        for one_test_batch in minibatch(run_queue, batch_size=batch_size):
            ...
            for ii, u_current in enumerate(user_id_batch_test):
                ...
                mrr = MRR_metric_timestep(actual_next_tokens[ii], predictions_logp[ii])
                # acc, ndcg = get_acc1(actual_next_tokens[ii], predictions_logp[ii])
                # mrr = MRR_metric_last_timestep(actual_next_tokens[ii], predictions_logp[ii])
                ...
                users_acc[u_current][7] += mrr  # @10
                users_acc[u_current][0] += (sequences_lens_batch[ii]-1)
                # users_acc[u_current][0] += 1
                users_acc[u_current][8] += 1
                # tmp_mrr[0] = users_acc[u_current][7]+tmp_mrr[0]
                # sum_mrr_test_samples = sum_mrr_test_samples+ users_acc[u_current][8]
                # users_rnn_acc6[u_current] = users_acc[u_current][7]
        ...
        # avg_mrr = (np.array(tmp_mrr)/sum_mrr_test_samples).tolist()
        # avg_acc.extend(avg_mrr)
        #---------------------old-------------------------
        users_rnn_acc0 = {}
        users_rnn_acc1 = {}
        users_rnn_acc2 = {}
        ...
        for u in users_acc:
            ...
            users_rnn_acc0[u] = tmps_acc0
            users_rnn_acc1[u] = tmps_acc1
            users_rnn_acc2[u] = tmps_acc2
            users_rnn_acc3[u] = tmps_acc3
            users_rnn_acc4[u] = tmps_acc4
            users_rnn_acc5[u] = tmps_acc5
            # sum_mrr_test_samples = sum_mrr_test_samples+ users_acc[u][8]
        ...
        avg_acc=[avg_acc0,avg_acc1,avg_acc2,avg_acc3,avg_acc4,avg_acc5,avg_mrr]
        return avg_acc
...
if __name__ == '__main__':
    ...
    # data = pickle.load(open('/home/xuxh/xuxh/LSTPM-master/LSTPM-master/train/foursquare_cut_one_day.pkl', 'rb'), encoding='iso-8859-1')
    data = pickle.load(open('../../dataset/baseline_models_dataset/LSTPM/tky.pk', 'rb'), encoding='iso-8859-1')
    ...
    poi_distance_matrix = pickle.load(open('../../dataset/baseline_models_dataset/LSTPM/tky_distance.pkl', 'rb'), encoding='iso-8859-1')
